...
class CustomerController extends Controller
{
    ...
    /* ---------- Shop ---------- */
        ...
        public function show_voucher(){
            ...
            return view("shop.customer.show-voucher")->with(compact('list_voucher','list_category','list_brand','hasBought'));
        }
        ...
        public function submit_login(Request $request){
            ...
            $login = Customer::where('username', $username)
            ...
            if($login){
                if($login->Status == 1){
                    ...
                    $hasBought = Bill::where('idCustomer', $login->idCustomer)->exists();
                    if (!$hasBought) {
        ...
        public function checkHasBought($idCustomer){
            return Bill::where('idCustomer', $idCustomer)->exists();
        }
        ...
        public function fetch_address() {
            $idCustomer = Session::get('idCustomer');
            $list_address = AddressCustomer::where('idCustomer', $idCustomer)->get();
            $default_address = Customer::where('idCustomer', $idCustomer)->first(['CustomerName', 'PhoneNumber', 'Address']);
            $output = '';
